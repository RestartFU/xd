  ...
  module UI
    class WorkflowCard
      ...
      def initialize(
        ...
        @status = Gtk::Label.new("Checking status…")
        @status.xalign = 0_f32
      ...
      private def finish_check(
        ...
        if snapshot
          set_status(snapshot.label, snapshot.css_class)
          schedule(POLL_INTERVAL) unless snapshot.terminal?
        else
          set_status("Status unavailable", "xd-workflow-finished")
          schedule(RETRY_INTERVAL)
        end
      end

      private def set_status(text : String, css_class : String) : Nil
        @status.text = text
