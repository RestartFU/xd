...
module Xd
  module Agent
    ...
    class Manager
      ...
      # Starts one private Codex turn for speech-to-text. It does not resume,
      # mutate, or append to the selected chat's assistant conversation.
      def transcribe_audio(
        chat_id : String,
        audio_path : String,
        finished : Proc(Bool, String?, String?, Nil),
      ) : SessionHandle
        @mutex.synchronize do
          raise Error.new("The daemon is stopping.") if @closed
        end
        ...
        spec = RunSpec.new(
          "Transcribe the attached audio accurately. Preserve technical " \
          "terms, capitalization, punctuation, commands, and file paths. " \
        ...
        on_event = ->(event : Event) {
          if event.type.text_delta? && (text = event.text)
            output_mutex.synchronize { output << text }
          end
          nil
        }
        on_finished = ->(ok : Bool, message : String?) {
          text = output_mutex.synchronize { output.to_s.strip }
          if ok && !text.empty?
            finished.call(true, text, nil)
          elsif ok
            finished.call(false, nil, "Codex returned no transcription.")
          else
            finished.call(
              false,
              nil,
              message || "Codex voice transcription failed."
            )
          end
        }
        ...
        raise Error.new(
          error.message || "Cannot start Codex voice transcription."
        )
      end
