  ...
  module Workspace
    ...
    class Worktrees
      ...
      def state(chat : Storage::Chat) : WorktreeState
        ...
        items = list(workdir)
        mark_generated_container(items)
      ...
      def create(
        ...
        if existing = worktrees.find do |item|
          ...
          mark_container(container) if within?(existing.path, container)
          return existing.path
        end

        mark_container(container)
        worktree_name = slug
        suffix = 2
      ...
      private def mark_container(path : String) : Nil
        Dir.mkdir_p(path, 0o700)
        marker = File.join(path, WORKTREE_CONTAINER_MARKER)
        return if File.file?(marker)

        File.write(
          marker,
          "Generated by XD for isolated Git worktrees.\n"
        )
        File.chmod(marker, 0o600)
      end

      # Older builds created this layout without identifying its top-level
      # container. Recognize only XD branches in XD's exact path convention,
      # then add the marker that keeps the container out of workspace scans.
      private def mark_generated_container(items : Array(Worktree)) : Nil
        ...
        mark_container(container) if generated
      end
