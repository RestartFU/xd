...
static const char *XD_STYLE =
  ...
  /* GtkText paints focused placeholders at the insertion cursor's x
   * coordinate, letting the caret cover the first glyph. Offset only the
   * placeholder node so entries and search fields keep their normal text
   * alignment without looking clipped while empty. */
