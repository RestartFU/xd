...
describe Xd::Voice::Model do
  ...
  it "follows the model host redirect to its download CDN" do
    directory = File.join(
      Dir.tempdir,
      "xd-voice-redirect-#{Random::Secure.hex(12)}"
    )
    path = File.join(directory, "speech", "model.bin")
    payload = ("redirected-model-data-" * 2048).to_slice
    digest = Digest::SHA256.hexdigest(payload)
    requests = [] of String
    target = ""
    server = HTTP::Server.new do |context|
      requests << context.request.resource
      if context.request.path == "/model"
    ...
    address = server.bind_tcp("127.0.0.1", 0)
    target = "http://127.0.0.1:#{address.port}/cdn/model?token=signed"
    spawn server.listen
